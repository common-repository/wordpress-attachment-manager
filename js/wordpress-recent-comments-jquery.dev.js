...
Author: Bharath
Update: 2010/10/14
Website: http://111waystomakemoney.com/
*/
...
RecentComment.prototype = {
	...
	init: function(config) {
		...
		this._buildList(this.param.initJson);
	},
			...
			jQuery(this).html(listCode).fadeIn(function() {
				...
				if(_self.param.showContent) {
					_self.context.list.find('li').each(function() {
						_self._bindCommentAction({item:jQuery(this)});
					});
				}
				...
				if(naviCode.length > 0) {
					_self._bindNaviAction({item:jQuery(this), pageNumber:json.navi.page});
				}
	...
	_bindCommentAction: function(args) {
		...
		if(itemEllipsis.length == 1) {
			itemExcerpt.parent().hover(function(ev) {
				_self._enterCommnet(ev, {_self:_self, item:item});
			},
			function(ev) {
				_self._leaveCommnet(ev, {_self:_self, item:item});
			});
		}
	...
	_enterCommnet: function(ev, args) {
		...
		if(itemToggle.length == 1) {
			...
			itemToggle.click(function(ev) {
				_self._toggleComment(ev, {_self:_self, item:item, source:itemToggle});
			});
